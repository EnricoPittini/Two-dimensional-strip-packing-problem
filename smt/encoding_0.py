...
class Vlsi_smt(Vlsi_smt_abstract):
    def __generate_encoding(self, l_max):
        ...
        # print(curr_abs_path)
        specific_instance_abspath = os.path.join(curr_abs_path, f'encoding_0_{instance_name}.smt2')
...
"""def generate_encoding(instance_name, w, n, dims):
    ...
    lines.append('(check-sat)')
    # String "(get-value ((coordX 0) (coordX 1) ... (coordX N) (coordY 0) (coordY 1) ... (coordY N)))"
    lines.append(f'(get-value ({" ".join([f"(coordX {i}) " for i in range(n)]) + " ".join([f"(coordY {i}) " for i in range(n)])}))')

    # FINALLY, CREATING THE FILE AND WRITING THE LINES
    with open(f'encoding_0_{instance_name}.smt2', 'w') as f:
        for line in lines:
            f.write(line + '\n')

    return os.path.abspath(f'encoding_0_{instance_name}.smt2')

#generate_encoding('PROVA', 4, 2, [2, 3], [2, 3])"""
from z3 import *
from sat_utils import exactly_one, UnsatError, Vlsi_sat_abstract, lex_lesseq_compound


class Vlsi_sat(Vlsi_sat_abstract):
    """Class for solving the VLSI problem in SAT, using the encoding 7B.

    It inherits from the class `Vlsi_sat_abstract`.

    Same solving and optimization of the encoding 4A.
    The only difference is that now some simmetry breaking constraints are ensured.

    In particular, we want to break only the horizontal flip of the plate simmetry.

    For ensuring this simmetry breaking constraint, the lexicographic ordering between lists of lists of boolean variables
    is used: see `lex_lesseq_compound` function.

    """
    def __solve(self, w_min, h_min, l_min, l_max):
        """Solves the given VLSI instance, using the SAT encoding 4B.
    ...
    def __optimize(self):
        """Solves the given VLSI instance, using the SAT encoding 7B.

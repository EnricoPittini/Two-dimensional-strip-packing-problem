...
class Vlsi_sat(Vlsi_sat_abstract):
    ...
    def __process_solver_instance(self, s, coords, lengths, w_min, h_min, l_min, l_max, current_best_l):
        ...
        #        print(lengths[k][l], m.evaluate(lengths[k][l], model_completion=True))
        l = max([l for k in range(n) for l in range(l_max-l_min+1) if m.evaluate(lengths[k][l], model_completion=True)])+l_min-1+1

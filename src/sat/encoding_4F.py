...
class Vlsi_sat(Vlsi_sat_abstract):
    """Class for solving the VLSI problem in SAT, using the encoding 4F.

    It inherits from the class `Vlsi_sat_abstract`.

    Same solving and optimization of the encoding 3.
    The only difference is that all the non-necessary constraints are removed.
    ...
    def __solve(self, w_min, h_min, l_min, l_max):
        """Solves the given VLSI instance, using the SAT encoding 4F.
        ...
        for k in range(n):
            for i in range(w-w_min+1):
                for j in range(l_max-h_min+1):
                    ...
                    THE EQUIVALENCE IS NOT NECESSARY"""
                    s.add(Implies(coords[i][j][k], And(no_overlapping_circuit_formula,all_positions_covered_formula)))
    ...
    def __process_solver_instance(self, s, coords, lengths, w_min, h_min, l_min, l_max, current_best_l):
        ...
        l = max([l for k in range(n) for l in range(l_max-l_min+1) if m.evaluate(lengths[k][l], model_completion=True)])+l_min

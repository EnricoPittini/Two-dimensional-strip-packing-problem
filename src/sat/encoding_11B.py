...
class Vlsi_sat(Vlsi_sat_abstract):
    """Class for solving the VLSI problem in SAT, using the encoding 11B.

    It inherits from the class `Vlsi_sat_abstract`.

    Like the encoding 11A, also this encoding solves the rotation variant of the VLSI problem.

    The solving procedure is the same of the encoding 11A. SAT variables 'px_i_e', 'py_i_f, 'lr_i_j', 'ud_i_j', 'r_i' and 'ph_o'.
    ...
    ---- PRACTICAL DIFFERENCES WITH ENCODING 11A ----
    In the encoding 11A, there is the binary search cycle, but at each iteration the solver is created and run from sratch,
    ...
    def __solve(self, l_min, l_max):
        """Solves the given VLSI instance, using the SAT encoding 11B.

...
class Vlsi_sat(Vlsi_sat_abstract):
    """Class for solving the VLSI problem in SAT, using the encoding 8B.

    It inherits from the class `Vlsi_sat_abstract`.

    Same solving and optimization of the encoding 4A.
    The only difference is that we use the HEULE encoding for the at_most_one constraint (in the exactly_one constraint).

    """
    def __solve(self, w_min, h_min, l_min, l_max):
        """Solves the given VLSI instance, using the SAT encoding 8B.
        ...
        for k in range(n):
            s.add(exactly_one([coords[i][j][k] for i in range(w-w_min+1) for j in range(l_max-h_min+1)], 
                              name=f'exactly_one_{k}', encoding='heule')) 

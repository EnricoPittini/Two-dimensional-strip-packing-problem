...
class Vlsi_sat(Vlsi_sat_abstract):
    """Class for solving the VLSI problem in SAT, using the encoding 4B.
        ...
        for k in range(n):
            for i in range(w-w_min+1):
                for j in range(l_max-h_min+1):
                    ...
                    # (THE EQUIVALENCE IS NON-NECESSARY, AN IMPLICATION WOULD BE ENOUGH)
    ...
    def __process_solver_instance(self, s, coords, lengths, w_min, h_min, l_min, l_max, current_best_l):
        ...
        s.add(Not(formula)) # NON-NECESSARY, BUT KEPT

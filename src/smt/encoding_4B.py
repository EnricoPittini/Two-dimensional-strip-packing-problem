...
class Vlsi_smt(Vlsi_smt_abstract):
    """Class for solving the VLSI problem in SMT, using the encoding 4B.

    It inherits from the class `Vlsi_smt_abstract`.

    It is exactly like the encoding 2C. SMT variables 'coordX[i]', 'coordY[i]' and 'l'. Linear search optimization procedure,
    ...
    This is a more specific logic with respect to the encoding 4A.
    ...
    def __generate_encoding(self, l_max):
        """Generates the SMT encoding for the specific instance of the VLSI problem, according to the encoding 4B.

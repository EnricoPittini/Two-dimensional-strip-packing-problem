...
class Vlsi_smt(Vlsi_smt_abstract):
    """Class for solving the VLSI problem in SMT, using the encoding 4F.

    It inherits from the class `Vlsi_smt_abstract`.

    It is like the encoding 4E. We are imposing the "QF_IDL" logic.

    The only difference is that now we make the constraints "more" compliant with the "QF_IDL" logic.
    In the "QF_IDL" logic, each atom must be in one of the following forms:
    - q
    - (op (- x y) n),
    - (op (- x y) (- n)), or
    - (op x y)
    ...
    def __generate_encoding(self, l_max):
        """Generates the SMT encoding for the specific instance of the VLSI problem, according to the encoding 4F.

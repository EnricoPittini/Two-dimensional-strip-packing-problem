...
class Vlsi_smt(Vlsi_smt_abstract):
    """Class for solving the VLSI problem in SMT, using the encoding 4D.

    It inherits from the class `Vlsi_smt_abstract`.

    It is exactly like the encoding 2C. Same SMT variables. Linear search optimization procedure, starting from the bottom 
    (i.e. l_min).
    ...
    This is a more specific logic with respect to the encoding 4A. Because now there aren't uninterpreted functions.
    ...
    def __generate_encoding(self, l_max):
        """Generates the SMT encoding for the specific instance of the VLSI problem, according to the encoding 4D.
